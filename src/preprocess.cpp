...
void Preprocess::robosense_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  if (!std::isfinite(first_point_time))
  {
    ROS_ERROR_THROTTLE(5.0,
        "RoboSense cloud has no valid per-point 'timestamp' field. Build rslidar_sdk with POINT_TYPE=XYZIRT.");
    return;
  }

  scan_start_time = first_point_time;
  const double header_error = std::fabs(msg->header.stamp.toSec() - first_point_time);
  if (header_error > 0.02)
  {
    ROS_WARN_THROTTLE(5.0,
        "RoboSense cloud header differs from its first point by %.6f s; using the first point time.",
        header_error);
  }
  if (feature_enabled)
  {
